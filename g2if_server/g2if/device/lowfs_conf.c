/* -------------------------------------------------------------------------
 * lowfs_conf.c -- Functions to configure parameters for device handler 
 *                of LOWFS serer
 * 
 * -------------------------------------------------------------------------
 * Update History:
 ...
 */
int lowfs_procconf(lowfs_t *lowfs, const char *sect, int argc,
		  const char **argv)
{
  char subsect[32];
...
{
  const char *header = lowfs->header;
  comm_t *comm = &(lowfs->comm);
